...
class CAPTURE:
    def __init__(
        ...
        if stop_words:
            from evaluation.model.stop_words import stop_words_list
        ...
        if self.soft_matching:
            self.text_encoder = SentenceTransformer(r"D:\models\all-mpnet-base-v2").to('cuda:0').eval()
    ...
    def parse_samples(self, samples, device, desc=""):
        ...
        if self.parser is not None and hasattr(self.parser, 'device') and self.parser.device == device:
            ...
            if torch.cuda.is_available():
                self.parser = SceneGraphParser(r"D:\models\flan-t5-base-VG-factual-sg", device=device)
            else:
                self.parser = SceneGraphParser(r"D:\models\flan-t5-base-VG-factual-sg", device='cpu')
...
if __name__ == '__main__':
    ...
    evaluator = CAPTURE()
    score = evaluator.compute_score(refs, preds)[0].item()
